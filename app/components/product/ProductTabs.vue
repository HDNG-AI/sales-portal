...
// Identifiers already spoken for by a visible PIM parameter. Plenty of
// catalogues express weight or a dimension as an ordinary parameter, and
// appending the product-level value regardless renders the same fact twice
// in one table — "500g" from the PIM beside "500 g" from here. Keyed on
// `identifier` for the same reason media classification is: it is the
// language-invariant name. Only visible parameters count, so a row hidden
// by its show flag still lets the product-level value through.
const parameterIdentifiers = computed(() => {
...
const measurementGroup = computed(() => {
  ...
  const parameters = [
    ...
    row.value > 0 &&
    !parameterIdentifiers.value.has(row.key)
      ? [
          {
            name: row.key,
