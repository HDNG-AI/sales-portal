...
/**
 * Same shape as CmsConfigSchema (store-settings.ts) but slot/menu values
 * are nullable — null means "remove this key" on update, matching
 * CmsConfigUpdate / mergeCmsConfig in server/utils/tenant-crud.ts. Kept
 * separate from CmsConfigSchema itself: that schema also validates the
 * merchant-API response, where a slot/menu value is never legitimately
 * null, and widening it there would let a malformed Geins response pass
 * validation instead of failing loudly.
 */
const CmsConfigUpdateSchema = z
  .object({
    slots: z
      .record(
        z.string(),
        z.object({ family: z.string(), areaName: z.string() }).nullable(),
      )
      .optional(),
    menus: z
      .record(z.string(), z.object({ menuLocationId: z.string() }).nullable())
      .optional(),
  })
  .optional();
