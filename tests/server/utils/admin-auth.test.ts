...
vi.mock('../../../server/utils/admin-config', () => ({
  readAdminAuthConfig: (...args: unknown[]) =>
    mockReadAdminAuthConfig(...args),
}));
...
const { timingSafeStringEqual, requireAdminAuth } = await import(
  '../../../server/utils/admin-auth'
);
...
describe('timingSafeStringEqual', () => {
  ...
  it('rejects a value that does not match', () => {
    expect(timingSafeStringEqual('wrong-secret', 'correct-secret')).toBe(
      false,
    );
  });
...
describe('requireAdminAuth', () => {
  ...
  it('rejects once the rate limit is exceeded, before checking the key', async () => {
    ...
    mockGetHeader.mockReturnValue('correct-secret');
    await expect(requireAdminAuth(mockEvent)).rejects.toThrow(
      'RATE_LIMITED',
    );
  });
});
