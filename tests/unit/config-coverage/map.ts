...
const HEADER_ACTIONS_NEW =
  'tests/components/layout/header/LayoutHeaderActionButtons.test.ts';
...
export const CONFIG_COVERAGE_MAP = {
  ...
  timezone: {
    ...
    test: [
      ...
      {
        spec: TENANT_DATE,
        title: 'omits the Intl option entirely when the tenant has no timezone',
        kind: 'consumer',
        drives: 'field',
      },
    ],
    note:
      'Every portal date site formats through formatTenantDate, so the two ' +
      'consumer references above cover all of them: the zone is passed ' +
      'straight to Intl as `timeZone`, or omitted when unset. Absent is not a ' +
      "fourth state — omitting the option is the runtime's own zone, and an " +
      'assertion on it would only restate where the test runs.',
  },
  ...
  productMediaParameters: {
    ...
    test: [
      ...
      {
        spec: PRODUCT_TABS,
        title: 'hides the documents tab when the product has no media',
        kind: 'consumer',
        drives: 'field',
      },
    ],
    note:
      'Fork-only until media is a first-class Geins citizen: the map says ' +
      'which parameter names carry video/document URLs, and ProductTabs ' +
      'pulls those out of the spec table into the Documents tab.',
  },
  ...
  theme: {
    ...
    colors: {
      ...
      // Two semantic status colours this fork adds on top of upstream's set,
      // for the stock states (see docs — added with the Odelco availability
      // work). Themeable like the rest, with a CSS fallback in tailwind.css.
      success: {
        status: 'has-test',
        test: [
          {
          ...
          {
            spec: STOCK_BADGE,
            title: 'renders in-stock state with semantic success styling',
            kind: 'consumer',
            drives: 'field',
          },
        ],
      ...
      warning: {
        ...
        test: [
          ...
          {
            spec: STOCK_BADGE,
            title: 'renders low-stock state with semantic warning styling',
            kind: 'consumer',
            drives: 'field',
          },
        ],
  ...
  features: {
    ...
    applyForAccount: namedFeature({
      ...
      on: [
        ...
        {
          spec: HEADER_ACTIONS_NEW,
          title: 'promotes account application when applyForAccount is enabled',
          kind: 'consumer',
          drives: 'field',
        },
        {
          spec: HEADER_TOPBAR,
          title:
            'does not render the applyForAccount anchor in the utility topbar',
          kind: 'consumer',
          drives: 'field',
        },
      ],
    ...
    priceVisibility: namedFeature({
      ...
      cells: visibilityCells(PRICE_DISPLAY, {
        ...
        anonymous:
          'offers login when priceVisibility requires authentication and the user is anonymous',
        signedIn:
